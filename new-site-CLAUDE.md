...
- **Locale-switch transition.** An in-app switch suppresses the page-entry animations for that mount (the `#main` rise's translateY flashes a paper-colored gap under the masthead) and instead re-types the first-viewport copy (`src/composables/retype.ts`, same ▌ caret as the briefs' typeout; ~0.65s, skipped for reduced-motion/hidden tabs). Wiring: router guard `markLangSwitch()` → view `consumeLangSwitch()` in `onMounted`. Opt an element out with `data-no-retype` (the toggle itself and the brand carry it).
