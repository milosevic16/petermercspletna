...
// Time-based (not frame-count), finishes in ~0.85s; skipped for
...
export function runLanguageRetype(): void {
  ...
  const STAGGER = 17.5 // ms between node starts, top-to-bottom
  const SPEED = 0.142 // chars per ms (time-based: throttled frames still finish)
